...
- **Übererfolge**: `extraSuccessEffect` am Zauber — `"DAMAGE"` = +2 Schadensstufen je Übererfolg (verrechnet); `"DURATION"` = **+2 Runden Effektdauer je Übererfolg, bei BUFF/DEBUFF mechanisch verrechnet** (`SpellService.effectiveDuration()`; permanent bleibt permanent, bei DAMAGE-Zaubern nur Anzeige); `"DURATION_MINUTES"` = +2 Minuten (= 20 Runden à 6 s) je Übererfolg (Blindheit); `"TARGET"` nur Anzeige. Phantomkrieger steht in der DURATION-Liste.
- **Verrechnet werden `EFFECT_STEP`** (+`value` auf die Wirkungsstufe) **, `BUFF_VALUE`** (verstärkt den Buff-/Debuff-Modifikator in Wirkrichtung; bei Phantomkrieger skaliert zusätzlich der hartkodierte Angreifer-Malus mit — „+1 Bild" = +1 KV **und** Angreifer −1 je Faden) **und `DURATION_ROUNDS`** (verlängert die Wirkungsdauer um `value` Runden je Faden; Blindheit: „+2 Minuten" = 20 Runden). **Nur `EFFECT_STEP` wird auf Schadens-/Heilstufen angewendet** (+`value` auf `effectStep`, wirkt in `applySpellDamage`/`applySpellHeal`). Alle anderen Optionen sind `DISPLAY`: sie werden gespeichert und im Log/Modal angezeigt, aber **nicht** gerechnet — die Engine kennt weder Reichweiten (kein Distanzsystem) noch Mehrfachziele (`SpellCastRequest.targetCombatantId` ist einzelzielig) noch Boni auf Nicht-Kampf-Proben (Heimlichkeit/Wahrnehmung sind keine `StatType`s). **Wirkungsdauer** ist bewusst `DISPLAY`: `SpellDefinition.duration` zählt **Runden**, die Regel nennt **Minuten**.
...
### Kampfkarte (Hexfeld-Map, optional)
Optionale räumliche Zusatzschicht pro Session — **fasst keinerlei Kampf-Logik an**. Ohne Aktivierung verhält sich alles exakt wie bisher; Reichweiten wirken **nur als UI-Filter** der Zielauswahl, das Backend validiert Angriffe/Zauber nicht räumlich.

- **Gitter**: odd-r-Hexfeld (spitze Hexes), `q` = Spalte, `r` = Zeile. Mathematik in `HexUtil.java` und gespiegelt in `frontend/src/app/services/hex-util.ts` (Distanz über kubische Koordinaten, Nachbarn parity-abhängig, BFS für Erreichbarkeit).
- **Datenmodell** (Flyway `V37`): `combat_sessions.map_enabled/map_width/map_height` · `combatant_states.map_q/map_r` (null = unplatziert) + `moved_hexes_this_round` (Reset in `nextRound()`) · Tabelle `map_obstacles` (`ObstacleType`: WALL/DOOR/TREE/ROCK/FURNITURE; nur offene Türen sind passierbar) · `characters.movement_hexes` (Bewegungsrate in Feldern, editierbar auf dem Charakterbogen via Feld-PATCH `movementHexes`) · `character_equipment.range_short/medium/long` (Felder, nur Projektil-/Wurfwaffen) · `spell_definitions.range_hexes` (0 = Selbst/Berührung; Migration setzt Selbst-Buffs/Heals auf 0, Rest Default 10).
- **`CombatMapService`**: `configureMap`, `placeCombatant` (q=-1 entfernt), `moveCombatant` (BFS-Wegkosten ≤ Restbudget, Hindernisse+Kombattanten blockieren, **nur wer laut Initiative dran ist** — erster nicht besiegter ohne `hasActedThisRound`; `gmOverride=true` umgeht Reihenfolge/Budget/Phase und kostet kein Budget), `addObstacle`/`removeObstacle`/`toggleDoor`. Jede Änderung → bestehender Session-Broadcast.
- **Endpoints**: `POST /sessions/{id}/map/configure?enabled=&width=&height=` · `POST .../map/place?combatantId=&q=&r=` · `POST .../map/move?combatantId=&q=&r=&gmOverride=` · `POST .../map/obstacles?type=&q=&r=` · `DELETE .../map/obstacles/{oid}` · `POST .../map/obstacles/{oid}/toggle-door`. Bewegungen loggen als `ActionType.MAP_MOVE` (im Tracker als System-Eintrag).
- **Kartenfenster**: Route `/combat/:id/map` → `CombatMapComponent`, geöffnet per „Karte"-Button im Tracker via `window.open` — eigener WebSocket-Subscribe, null Umbau am Tracker. SVG-Hexgitter, vertikal gestaucht (SQUASH 0.82, Pseudo-Schrägsicht; Ebenen später ergänzbar). GM-Palette (Kombattanten platzieren, Hindernisse, Radierer; GM = kein aktiver User oder `gamemaster=true`), Token-Auswahl → erreichbare Felder (BFS clientseitig identisch) → Klick bewegt (CSS-Transition). **Angriffs-Animationen** aus neuen Log-Einträgen (Hieb/Pfeil/Zauberbolzen je `ActionType`), Zuordnung über `actorName`/`targetName`.
- **Zielauswahl im Tracker** (`filterByMapRange`): nur wenn Karte aktiv und Akteur platziert — Nahkampf/Manövrieren/Zweitwaffe: angrenzend (Distanz 1) · Fernkampf: ≤ `rangeLong` der gewählten Waffe (ungepflegte Reichweite = kein Filter) · Zauber: ≤ `spell.rangeHexes` (0 → angrenzend). Unplatzierte Ziele bleiben immer wählbar.
- **Aktivierung**: Checkbox beim Anlegen (combat-list, ruft nach dem Create `map/configure` auf — der Create-Endpoint blieb unverändert) oder Button „Karte aktivieren" im SETUP.
...
### Blindheit (Zauber, Illusionist Kreis 2)
- **Effekt**: −4 auf alle Proben (als `ATTACK_STEP −4 ALWAYS` — gleiche Näherung wie Verängstigen), Basis 3 Runden.
- **Dauer**: Übererfolge (`DURATION_MINUTES`) und der Zusatzfaden „+2 Minuten" (`DURATION_ROUNDS` 20) verlängern um je 20 Runden.
- **Durchschauen**: `CombatService.durchschauenCheck(session, roller, total)` — würfelt der Geblendete bei irgendeiner Aktionsprobe **über 17** (der −4-Malus steckt bereits im Wurf), wird der Effekt `TalentNames.EFFECT_BLINDHEIT` entfernt und als `EFFECT_REMOVED` geloggt. Der Check hängt an **allen 15 Aktionswurf-Stellen**: performAttack, resolveDodge, aufspringen, Taunt, Fear, NeutralizeMagic (2×), CombatSense, Distract, SpotArmorFlaw (2×), FreeAction, Manoeuver sowie weaveThread und castSpell im SpellService. Initiative- und Schadenswürfe zählen nicht.

### Geisterbeschwörer-Zauber (mechanisiert)
- **Geisterpfeil**: Schaden (WIL + Wirkungsstufe, mystische Rüstung) **plus MR-Debuff −2** auf dem Ziel (Sonderfall in `applySpellDamage`, ersetzt bei erneutem Wirken; gilt für beide Seed-Varianten, auch Illusionist). Basisdauer 2 Runden, `DURATION`-Übererfolge +2 Runden je. Fäden: `EFFECT_STEP` +2 Schaden, `BUFF_VALUE` +2 MR-Senkung, Reichweite/Ziel nur Anzeige. Übererfolge geben **keinen** Schadensbonus.
- **Nebelschild**: neuer `StatType.DODGE_STEP` (Basiswert 0) — der Bonus wirkt **nur auf die Ausweichen-Reaktionsprobe** (`resolveDodge` addiert den effektiven `DODGE_STEP`), nicht mehr als pauschales +KV. Faden `BUFF_VALUE` +2, Übererfolge `DURATION`.
- **Schmerzen**: −3 auf alle Proben (ATTACK_STEP-Näherung wie Verängstigen, „3 temporäre Wunden") **plus halbe Bewegungsrate**: zweiter Modifikator `MOVEMENT_HEXES MULTIPLY 0.5` (Sonderfall in `applySpellDebuff`, skaliert nicht mit Fäden). Faden `BUFF_VALUE` +1 Wunde (→ −4 …), Übererfolge `DURATION`.
- **Schädel des Todes**: Effekt auf dem **Zauberer** (`TalentNames.EFFECT_SCHAEDEL_DES_TODES`, Sonderfall in `applySpellBuff`): **Verängstigen kostet keine Hauptaktion**; Dauer = Spruchzauberei-Rang + 5 (+2 je Übererfolg). Faden `BUFF_VALUE` +2 auf Verängstigen-Proben — gespeichert als `ON_SOCIAL_ACTION`-ModifierEntry am Effekt, `performFear` liest ihn aus. **Achtung Verhaltensfix**: `performFear` verbraucht jetzt ohne den Effekt die Hauptaktion (vorher gar nicht — Doku sagte Standardaktion).
- **`StatType.MOVEMENT_HEXES`**: Basis = `characters.movement_hexes`; `CombatMapService.effectiveMovement()` nutzt den Aggregator fürs Bewegungsbudget, das Kartenfenster spiegelt die Effekt-Auswertung clientseitig (`effectiveMovement()` in `CombatMapComponent`).
...
- **Karma auf Schaden**: `AttackActionRequest.spendKarmaForDamage` aktiviert einen zusätzlichen W6 (Stufe 4) auf den Schadenswurf. Erlaubt bei `clawWeapon=true` (Krallenhand, disziplinunabhängig) ODER disziplinabhängig über `CombatService.karmaOnDamageAllowed(attacker, weaponIsClaw, attackSourceName)`: **Krieger** ab Kreis 5 im Nahkampf (Nahkampfwaffen/waffenlos), **Schütze** ab Kreis 5 im Fernkampf (Projektil-/Wurfwaffen), **Schwertmeister** Nahkampfwaffe, **Luftpirat** Nahkampf-/Wurfwaffen (Größenbedingung mangels Waffengröße nicht geprüft), **Tiermeister** waffenlos. Der Angriffsquellen-Name (Talent/Fertigkeit) bestimmt die Waffenart. Frontend spiegelt die Regel in `canUseKarmaForDamage()`. Der Check greift nur im Trefferzweig — bei Fehlschlag wird kein Karma verbraucht. Das Ergebnis wird als `damageKarmaRoll` in `CombatActionResult` zurückgegeben und im UI separat angezeigt.
