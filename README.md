...
- **Six more non-combat talents** — Erster Eindruck, Gefühlsmelodie, Etikette (CHA), Empathische Wahrnehmung, Forschen, Fremdsprachen (PER). Testable utility talents, seeded idempotently in migrateUtilityTalents, no combat relevance.
- **Thread weaving for every discipline** — non-magic disciplines now each get a plain `Fadenweben (<Discipline>)` talent (WAH-based, testable, no combat relevance: weaving threads to items/patterns). Seeded idempotently for every discipline that lacks a dedicated spell-weaving talent (the four magic ones keep Elementarismus/Illusionismus/Magie/Geisterbeschwörung) and appended to each discipline access list. 11 talents on the current data set.
...
- **Four Geisterbeschwörer spells mechanized** — **Geisterpfeil** now also lowers the target's mystic armor by 2 (base 2 rounds, +2 per extra success; threads: +2 damage step, further −2 MR); **Nebelschild** grants its bonus only on the dodge reaction test (new `DODGE_STEP` stat) instead of flat +KV, +2 per thread; **Schmerzen** applies −3 on all tests ("3 temporary wounds", +1 per thread) **and halves the movement rate on the combat map** (new `MOVEMENT_HEXES` stat, MULTIPLY 0.5); **Schädel des Todes** becomes a real effect on the caster (Spruchzauberei rank + 5 rounds, +2 per success): Verängstigen no longer consumes the main action and gains +2 per thread. **Behavior fix**: without the skull, Verängstigen now correctly consumes the main action (it previously consumed none despite being documented as a standard action). Existing databases migrate idempotently.
- **Blindheit rework** — the spell now imposes **−4 on all tests** (was −3 attack only; same ATTACK_STEP approximation as Verängstigen). Extra successes and the "+2 Minuten" extra-thread option each **extend the duration by 2 minutes (20 rounds)** — new computed option type `DURATION_ROUNDS` and success effect `DURATION_MINUTES`. **Seeing through the illusion**: whenever the blinded combatant rolls **over 17** on any action test (attack, dodge, spell, talent, free action — the −4 is already in the roll), the effect ends immediately with a log entry (`durchschauenCheck`, wired into all 15 action-roll sites). Existing databases migrate idempotently.
- **Phantomkrieger scales with successes and threads** — extra successes on the casting roll now **extend the duration by 2 rounds each** (implemented generically: `extraSuccessEffect="DURATION"` is applied mechanically for BUFF/DEBUFF spells — the other spells in that list benefit too). The "+1 Bild" extra-thread option became a computed option (new type `BUFF_VALUE`): each woven image grants **+1 KV on the target and −1 on attacks against it**, on top of the base +3/−3. Existing databases are migrated idempotently at startup.
- **Fix — equipment forms clipped their fields** — the flexible fields (Name, Beschreibung) in the add-equipment rows had no minimum width, so fixed-width neighbours (e.g. the new Kurz/Mittel/Weit range inputs) squeezed them to slivers instead of wrapping. All flex fields now carry a flex-basis + min-width and wrap onto a second row.
- **Fix — combat log was oldest-first** — the backend already delivers the log newest-first (`@OrderBy("id DESC")`), and the frontend's `toLogEntries` reversed it again, flipping the list to oldest-first. Now sorted explicitly by id descending regardless of input order.
- **Hex-grid combat map (optional)** — a spatial layer on top of the existing combat system that leaves every existing flow untouched. Enable it per session (checkbox at creation or a SETUP button); a **"Karte" button opens the map in its own window** (`/combat/:id/map`) fed by the same WebSocket broadcast. The GM places combatants and obstacles (walls, doors — openable, trees, rocks, furniture) on an odd-r hex grid (size configurable, default 24×16, pseudo-oblique 2D view; levels can come later). Combatants move **in initiative order** up to their per-round **movement rate** (new character-sheet field), validated server-side via BFS path costs around obstacles — the GM may move anyone anytime. Melee requires adjacency, ranged weapons get **short/medium/long ranges in hexes** (new weapon fields), spells get a **range in hexes** (`range_hexes`, seeded: self-buffs 0, rest 10) — ranges act purely as a **UI filter on target selection**; backend combat validation is deliberately untouched. Attacks animate on the map (slash/arrow/spell bolt) driven by new combat-log entries. Flyway `V37`.
- **App icon / favicon** — the app finally has a mark of its own: a **thread ring pierced by a sword** (the ring for thread weaving, Earthdawn's core mechanic; the sword for combat), drawn in the app's gold `#c9a84c` on `#17120d`. It is an original design, not the FASA logo. One `frontend/public/favicon.svg` serves both the browser tab and the sidebar logo; `favicon.ico` (16/32/48) is the fallback for browsers without SVG-favicon support. Deliberately built from bold shapes rather than fine lines so it survives being scaled down to 16 px.
- **Fix — three icons rendered as raw text** — the app loads the *classic* Material Icons font, which has no `sentiment_extremely_dissatisfied` (Verängstigen button), `skull` (enemy header, defeated marker, unconscious banners) or `shield_outlined` (accounts, combat list). An unknown ligature makes `mat-icon` print the literal name, which filled the button and pushed its label out of view — hence "Verängstigen has no label". Replaced with `mood_bad`, `dangerous` and `add_moderator`/`remove_moderator`. `frontend/scripts/check-mat-icons.py` validates every ligature against the font and fails on unknown names; the build cannot catch this.
- **All declaration buttons now have icons** — *Neutral*, *Aggressiv*, *Defensiv*, *Waffe*, *Zauber* and *Ändern* used emoji or nothing; they now use real `mat-icon`s like every other button.
- **Combatant card is now two-column** — the card header (initiative, name, discipline, badges, *Auto*) spans the full width; below it the left column holds the values (damage track, wounds/karma, defenses, armour, active effects) and the right column stacks all action buttons vertically. **Every action button is now labelled** (previously most were icon-only with a tooltip); long names are ellipsised rather than overflowing. *Furcht abschütteln* lost its glaring Material accent green for a muted teal matching the other buttons.
- **Free extra thread from an Erweiterte Matrize** — an **instant spell (0 threads)** held in an Erweiterte Matrize turns the matrix's pre-woven thread into a free extra thread: always **Wirkungsstufe +2**, no roll, no action, and **not** counted against the weaving-rank cap. Spells without an effect-step option (e.g. Katastrophe) get nothing; spells that require threads already consume the matrix thread via the `−1` discount.
- **Extra threads on spells (`Zusatzfäden`)** — once all required threads are woven, each further thread buys exactly one of the spell's listed options (seeded for 10 Illusionist spells). Only **Wirkungsstufe (+2 effect step)** is applied automatically; every other option (range, extra targets, duration, non-combat bonuses) is **recorded and displayed for the GM to adjudicate**, because the engine has no distance system, casts are single-target, `duration` counts rounds while the rule states minutes, and stealth/perception tests aren't modelled stats. Capped at the **weaving talent's rank**; the same option may be picked repeatedly. **Instant spells (0 threads) can take extra threads too** (e.g. Blitz). Flyway `V36`.
- **New talent: Magie neutralisieren** — ends any single active effect on any combatant. WIL + rank vs. **effect level + 10**; consumes the round's action and 1 strain. Because effects carry no level of their own (the originating spell/talent does), the level is entered when applying. Uses a **two-step, fully broadcast flow**: applying opens a selection dialog **on every client** (WebSocket) listing all current effects across all combatants; the result modal is likewise broadcast. Which effects are actually neutralizable is left to the GM — all are offered.
